...
public final class HexUtils {
    ...
    public static long parseAddress(String text) {
        ...
        if (value.isBlank()) throw new IllegalArgumentException("Address is empty");
        if (value.startsWith("0x")) return Long.parseUnsignedLong(value.substring(2), 16);
        if (value.startsWith("$")) return Long.parseUnsignedLong(value.substring(1), 16);
    ...
    public static long parseUnsigned32(String text) {
        ...
        if ((value & ~0xFFFF_FFFFL) != 0) throw new IllegalArgumentException("Value does not fit in 32 bits");
        return value;
    }
    ...
    public static byte[] parseHex(String text) {
        ...
        if (cleaned.isBlank()) return new byte[0];
        if ((cleaned.length() & 1) != 0) throw new IllegalArgumentException("Hex must contain full bytes");

...
final class ResumableUploader {
            ...
            while (true) {
                ...
                if (reconnectNeeded) {
                    ...
                    try {
                        ...
                        if (!failure.retryable()) throw transferFailure(failure);
                        continue;
                    }
                }
                ...
                try {
                    ...
                    if (present < verified) verified = alignedCheckpoint(present);
    ...
    private static void writeAndVerify(RemoteFile remote, String remotePath, long offset, byte[] data)
        ...
        if (Arrays.equals(data, actual)) return;
    ...
    private static byte[] readLocal(FileChannel local, long offset, int length) throws IOException {
        ...
        while (buffer.hasRemaining()) {
            ...
            if (read < 0) throw new IOException("Local file ended unexpectedly at byte " + position);
            if (read == 0) continue;
            position += read;
        }
        return buffer.array();
